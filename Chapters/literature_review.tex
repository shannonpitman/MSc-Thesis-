...
Art Gallery problem -> how many guards, wich can survey a full 360 degrees is the minimum to cover the room shaped as a simple polygon. Obvious assumptions of two-dimensions, full 360 degrees field of view, no limited range of sight. Thus, it can be extended to the camera placement problem by adding constraints 
Marker based tracking so position of markers can e tracked \parencite{zieglerOptimalCameraPlacement2023} - in general markers must be visible by at least two cameras in order to calc their respective 3D positions via triangulation and thus motion can be tracked \parencite{aissaouiDesigningCameraPlacement2018}  
Need application domains from notes: 
VSNs (Visual Sensor Networks) are used in the following industries 
1. surveillance \parencite{bodorOptimalCameraPlacement2007}
2. object detection
3. motion capture \parencite{aissaouiDesigningCameraPlacement2018, zieglerOptimalCameraPlacement2023}
4. Augmented and virtual reality systems \parencite{rahimianOptimalCameraPlacement2017} 
5. Autonomous Driving \parencite{puligandlaMultiresolutionApproachLarge2022}
While building visual sensor networks, variables like sensor number, location, and orientation (i.e sensor placement optimisation problem) have a significant impact on coverage and performance of the system to identify and monitor the target \parencite{zhouSensorPlacementOptimization2024}. 
